...
public class WsAgent {
		...
		this.transport.addMessageHandler(new WebsocketTransport.MessageHandler() {
			public void handleMessage(byte[] bytes) throws WebsocketError {
				if (!isHandshakeCompleted) {
					...
					try {
						GatewayHandshakeMessage gatewayHandshakeMessage = objectMapper.readValue(bytes,
								GatewayHandshakeMessage.class);
				...
				try {
					...
					WebsocketMessage websocketMessage = objectMapper.readValue(incomingMessage.content,
					...
					if (websocketMessage.sequenceNum == incomingSequenceNum) {
						...
						throw new WebsocketError(
								"Received message sequence number does not match next expected value. Expected: "
										+ incomingSequenceNum + ", received: " + websocketMessage.sequenceNum);
	...
	void sendApplicationMessage(byte[] payload, boolean isServiceMessage) {
		...
		WsProxy wsProxy = ProxyBuilder.create(this.agent, this.canisterId)
				.expireAfter(Duration.ofMillis(DEFAULT_INGRESS_EXPIRY_DELTA_IN_MSECS)).getProxy(WsProxy.class);
	...
	void sendKeepAliveMessage() {
		WebsocketServiceMessageContent websocketServiceContent = WebsocketServiceMessageContent.KeepAliveMessage;
		...
		websocketServiceContent.keepAliveValue = keepAliveMessageContent;

		try {
			byte[] payload = this.encodeValue(websocketServiceContent);

			LOG.debug("Sending keep alive message for client " + this.clientKey.clientPrincipal.toString() + " nonce "
					+ this.clientKey.clientNonce);
	...
	boolean handleSystemMessage(byte[] serviceMessageContent) {
		try {
			WebsocketServiceMessageContent result = IDLArgs.fromBytes(serviceMessageContent).getArgs().get(0)
					.getValue(new PojoDeserializer(), WebsocketServiceMessageContent.class);
